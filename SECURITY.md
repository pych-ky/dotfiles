...
- `gh` / `docker` / `terraform` / `terragrunt` / `git commit` / `git push` はサンドボックス外で実行する。
  `gh` は認証、`docker` はソケット接続、`git commit` は pre-commit フックの secretlint が docker ソケットへ接続するため、`git push` は credential helper の `gh` が `~/.config/gh/hosts.yml` を読むために必要になる。
  ...
  `git commit` / `git push` は Git フック経由で任意のコマンドをサンドボックス外で実行しうるため、グローバルフックと各リポジトリの `.git/hooks` を信頼できる範囲に保つ前提で受容する
