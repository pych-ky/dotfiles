...
macOS の OS 設定、シェル、Homebrew、開発用 CLI、Git、AI エージェントをセットアップする個人用 dotfiles です。
...
Homebrew（未導入時は Xcode Command Line Tools も含む）と、不足するアプリ・開発ツールを導入します。
冒頭に `sudo` 認証を行います。
Homebrew は認証キャッシュを無効化するため、アプリの導入や `/etc/codex/config.toml` の初回作成時には再認証が必要になる場合があります。
途中で失敗した場合は、最後に表示される失敗内容を解消して再実行できます。
`skipped steps` は未実行の処理です。
終了状態が 0 でも確認し、必要な認証や前提を揃えて再実行してください。
終了後は「[手動セットアップ](#手動セットアップ)」を行ってください。

設定の多くはこのリポジトリへのシンボリックリンクで、リンク後の編集は実環境にも反映されます。
...
- システム設定 > プライバシーとセキュリティ
  - 必要なアプリだけにフルディスクアクセス・アクセシビリティ・入力監視を許可する
  - ログイン項目の登録には、オートメーションで System Events を許可する
- システム設定 > 一般 > ログイン項目と拡張機能
  - 「ログイン時に開く」: Maccy、Rectangle、Typeless があり、Logi Options+ がないことを確認する
...
- Rancher Desktop: リンク先リポジトリへの自動追記を防ぐため、Preferences > Application > Environment > Configure PATH を Manual にする
- [Maccy](https://github.com/p0deje/Maccy#usage): `Cmd+Shift+C` で履歴を開き、自動貼り付けには「Paste automatically」とアクセシビリティを許可する
...
- `bootstrap.sh` で導入する Claude Code の Linear / Microsoft Docs と Codex CLI の Linear は、Linear の OAuth 認証を端末ごとに完了する
...
組織固有の URL に限って [ghtkn](https://github.com/suzuki-shunsuke/ghtkn) の helper を使う端末は、非公開側の設定に従って追加で認証してください。
ghtkn は mise で導入します。
...
**このリポジトリは公開です。**
組織固有の設定と公開したくない個人情報は非公開の `dotfiles-private` で管理します。
`bootstrap.sh` はアクセス可能な場合だけ取得し、その `setup.sh` を実行します。
不要なら `DOTFILES_PRIVATE_SKIP=1` を指定してください。
...
- 認証情報そのものは非公開リポジトリにも置かず、1Password などで別途移行する
- GitHub 認証は端末ごとに取り直す
...
クレデンシャルをモデル・会話・tool output へ直接取り出すことは禁止です。
認証済み CLI が内部で認証情報を利用・保管する通常操作は許可します。
...
| 設定 | 配置と再適用 |
| --- | --- |
| `~/.claude/settings.json` | コピーし、既存設定のマージ時は公開設定を優先（`jq` が必要）。公開側に同じ ID がない個人のプラグイン・marketplace 登録は保持 |
| `~/.codex/browser/config.toml` | コピー |
| `/etc/codex/config.toml` | リンクし、端末固有の `~/.codex/config.toml` で上書き可能 |

端末固有設定の旧 `sandbox_mode` / `[sandbox_workspace_write]` は削除してください。
`sandbox_mode` が残ると公開側の `default_permissions` が使われません。

#### Codex App の権限

リンク後は Codex を終了し、承認メニューで「保護付きフルアクセス」を選んで新しいタスクを開始してください。
組み込みの「フルアクセス」は `approval_policy = "never"` で上書きするため、承認が必要な Computer Use のアプリ接続も拒否されることがあります。
既存タスクはプロファイルを切り替えても承認方式を引き継ぐため、`approval_policy = "on-request"` が適用されたとは限りません。
`on-request` は必要な承認を求められる設定で、全操作を毎回確認するものではありません。

最後に選んだ権限は `~/.codex/.codex-global-state.json` に保存され、`default_permissions` より優先されます。
この選択は `[desktop]` では管理できないため、端末ごとに App で設定します。
...
名前やメールの自動推測は無効のため、[非公開設定](#非公開設定dotfiles-private)で設定してください。
...
secretlint が未導入・実行不能の場合もコミットを止めます。
単独で導入する場合は、設定のリンク後に実行してください。

```sh
mise install aqua:secretlint/secretlint
```

組織固有の文字列を検査する denylist は非公開側で設定します。
`denylist.txt` がない端末ではこの検査は行われません。
...
TFLint は mise で管理します。
旧 Homebrew 版から移行する場合は `mise install tflint` と `mise exec -- tflint --version` が成功してから、`brew uninstall --cask tflint`、`brew untap terraform-linters/tap` を実行してください。
一部の GUI アプリは [Brewfile](macos/Brewfile) でコメントアウトしています。
必要に応じて別途導入するか、コメントアウトを外して `brew bundle` を再実行してください。
...
| 操作 | 動作 |
| --- | --- |
| `cghq`（Zsh では `Ctrl+G` も可） | ghq 管理のリポジトリを検索し、Enter で移動。`cghq dotfiles` のように初期検索語を指定可能 |
| `Ctrl+R` | コマンド履歴を検索 |
| `Ctrl+T` | ファイル・ディレクトリを検索し、パスを入力 |
| `Alt+C` | ディレクトリを検索して移動 |
| `cd **` などの末尾で Tab | パスをファジー補完 |
...
Typeless は macOS と `jq` が必要です。
[管理する設定](macos/typeless.json)だけを反映し、アカウント別設定や機器情報は保持します。
設定を変更するときは Typeless を終了してください。
旧ショートカットの移行を求められた場合は、一度起動・終了してから再実行してください。

### キーボード

共通のキー変換は [Karabiner](.config/karabiner/karabiner.json)、ターミナルは [WezTerm](.wezterm.lua) で管理します。

- 左 Control / Option / Command は Command / Control / Option に、Caps Lock は Control に変わる
- 右 Command / Option で、かな / 英数を切り替える
- `Cmd+Space` で Spotlight の代わりに Raycast を開く
- WezTerm・VS Code 統合ターミナルの `Cmd+C` は、選択中はコピー、未選択時は処理中断

`Cmd` は OS に送るキーで、Windows の左 Ctrl の位置から操作できます。

外付けキーボードは Mac モードで使用してください。
Windows 配列だけの機器は、Karabiner の機器別 Simple Modifications で `left_command → left_control`、`left_option → left_option` を指定します。
...
既存のチェックアウトは自動更新しません。
更新する場合は以下を実行し、dotfiles に戻って `./pets/setup.sh` を再実行します。
...
同名の未管理オブジェクトは上書きしません。
Claude のクラウドルーティンの初回登録・変更は、Agent Skills 側の `routines/claude-app/sync.sh` を明示実行します。
...
`delegate-to-chatgpt` は通常 ChatGPT への内蔵送受信ツールがある Codex デスクトップ向けです。
自動委譲する資料の許可範囲と配置手順は、同 README の「通常 ChatGPT への自動委譲」に従ってください。
