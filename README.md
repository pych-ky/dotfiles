...
個人環境向けの dotfiles です。
`bootstrap.sh` が Homebrew の導入からパッケージ・シンボリックリンク・macOS 設定の適用までを一括で行います。
...
1. Homebrew を導入する (未導入時、Xcode Command Line Tools も同時に導入)
2. `Brewfile` に基づいて CLI / GUI アプリ / npm グローバルを一括インストールする
3. zsh プラグイン (zsh-autosuggestions, fast-syntax-highlighting) を取得する
4. `install.sh` で dotfiles のシンボリックリンクを展開する
5. `macos/defaults.sh` で macOS 設定を適用する
6. Claude Code CLI を導入する (未導入時)
